...
class YOLOV7_OPENVINO(object):
    def __init__(self, model_path, device, pre_api, batchsize, nireq):
        ...
        self.stride = [8, 16, 32]
        self.anchor_list = [[12, 16, 19, 36, 40, 28], [
            36, 75, 76, 55, 72, 146], [142, 110, 192, 243, 459, 401]]
        self.anchor = np.array(self.anchor_list).astype(
            float).reshape(3, -1, 2)
        area = self.img_size[0] * self.img_size[1]
        self.size = [int(area / self.stride[0] ** 2), int(area /
                                                          self.stride[1] ** 2), int(area / self.stride[2] ** 2)]
        self.feature = [[int(j / self.stride[i])
                         for j in self.img_size] for i in range(3)]

        ie = Core()
        self.model = ie.read_model(model_path)
        self.num_output = self.model.get_output_size()
    ...
    def draw(self, img, boxinfo):
        for xyxy, conf, cls in boxinfo:
            self.plot_one_box(xyxy, img, label=self.classes[int(
                cls)], color=self.colors[int(cls)], line_thickness=2)
    ...
    def postprocess(self, infer_request, info):
        ...
        for batch_id in range(self.batchsize):
            if self.num_output == 1:
                ...
                output.append(self.sigmoid(infer_request.get_output_tensor(
                    0).data[batch_id].reshape(-1, self.size[0]*3, 5+self.class_num)))
                output.append(self.sigmoid(infer_request.get_output_tensor(
                    1).data[batch_id].reshape(-1, self.size[1]*3, 5+self.class_num)))
                output.append(self.sigmoid(infer_request.get_output_tensor(
                    2).data[batch_id].reshape(-1, self.size[2]*3, 5+self.class_num)))

                # Postprocessing
                grid = []
                for _, f in enumerate(self.feature):
                    grid.append([[i, j] for j in range(f[0])
                                for i in range(f[1])])
                ...
                for i in range(3):
                    ...
                    for j in range(3):
                        dst_xy.append(
                            (xy[:, j * self.size[i]:(j + 1) * self.size[i], :] + grid[i]) * self.stride[i])
                        dst_wh.append(
                            wh[:, j * self.size[i]:(j + 1) * self.size[i], :] * self.anchor[i][j])
    ...
    def infer_cam(self, source):
        ...
        while (cap.isOpened()):
            ...
            # Do inference
            self.infer_queue.start_async(
                {self.input_layer.any_name: img_batch}, (src_img_list, src_size))
            src_img_list = []
            img_list = []
