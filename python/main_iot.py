...
class YOLOV7_OPENVINO(object):
    ...
    def plot_one_box(self, x, img, color=None, label=None, line_thickness=None):
        ...
        cv2.rectangle(img, (xmin, ymin), (xmax, ymax), color, thickness=tl, lineType=cv2.LINE_AA)

        (w, h), _ = cv2.getTextSize(
            f"{label:.2f}", cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1
        )
        
        # Prints the text.
        cv2.rectangle(
            img, (xmin, ymin + h + 5), (xmin + w + 5, ymin), (255, 255, 255), -1
        )
    ...
    def insert_people_count(self,image, people):
        ...
        cv2.putText(
            image,
            f"Fps: {str(round(routes.current_fps, 2))}",
            (1, 50),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.8,
            (0, 0, 0),
    ...
    def infer(self):
        ...
        while True:
            success, src_img = video_capture.read()
            
            if not success:
                print("ERROR! No video feed, breaking out of script")
                break
            ...
            output.append(
                self.sigmoid(
                    pred[self.compiled_model.output(0)].reshape(
                        -1, size[0] * 3, 5 + self.class_num
                    )
                )
            )
            output.append(
                self.sigmoid(
                    pred[self.compiled_model.output(1)].reshape(
                        -1, size[1] * 3, 5 + self.class_num
                    )
                )
            )
            output.append(
                self.sigmoid(
                    pred[self.compiled_model.output(2)].reshape(
                        -1, size[2] * 3, 5 + self.class_num
                    )
                )
            )
            ...
            frame_number += 1
            routes.current_fps = round(frame_number / total_time, 5)
